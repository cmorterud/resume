...
\begin{itemize}  \itemsep -2pt %reduce space between items
    ...
    \item Decreased service latency by refactoring external
    API calls into separate threads and validating changes
    through performance testing.

    \item Ensured production deployment stability by implementing, 
    and designing acceptance tests with Cucumber.

    \item Deployed new applications to AWS by
    understanding and configuring in-house deployment software.
    
    \item Designed complete suites of custom alerts through Kibana
    for production incidents for multiple services.

    \item Wrote documentation for services owned by team
    to help onboard feature teams to our applications.

    \item Refactored applications for redundancy on errors
    and graceful failures.
...
\begin{parcolumns}[rulebetween,colwidths={5=2cm,1=2cm}]{6}
    \colchunk[1]{\textbf{Languages}\\Java\\C/C\texttt{++}\\Python\\C\texttt{\#} .NET\\\LaTeX}
    \colchunk[2]{\textbf{IDE}\\IntelliJ\\Visual Studio}
    \colchunk[3]{\textbf{Technologies}\\Git\\Bash\\Kibana\\Grafana}
    \colchunk[4]{\\Flask\\Perf\\MapReduce\\Postman}
    \colchunk[5]{\\WPF\\GDB\\Valgrind\\Make\\PyTorch}
    \colchunk[5]{\textbf{Java}\\Spring\\Maven\\Mockito}
    \colchunk[6]{\textbf{AWS}\\CloudWatch\\Lambda\\ECS\\RDS}
\end{parcolumns}

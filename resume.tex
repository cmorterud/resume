...
\usepackage{hyperref}
\usepackage{enumitem}
\setlength{\textwidth}{7.45in} % set width of text portion
...
\begin{itemize}  \itemsep -2pt %reduce space between items
    ...
    company-wide C\texttt{\#} .NET Nuget package to add
...
\begin{itemize}[label={}]  \itemsep -2pt %reduce space between items
    ...
    \item \textbf{C\texttt{++}, CMake, Zookeeper, gRPC, Azure}
        Developed and tested scalable, parallel MapReduce framework implementation in C\texttt{++} serving jobs in Azure
        on infrastructure built with Azure Resource Manager templates.
